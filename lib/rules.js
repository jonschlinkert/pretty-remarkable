...
rules.link_open = function(tokens, idx /*, options, env */) {
  ...
  if (close.type === 'link_close' && after && after.type === 'text') {
    if (after.content && after.content.slice(0, 2) === '{#') {
      token.href += after.content.slice(1, after.content.length - 1);
      after.content = '';
    }
  }
  ...
  return '[' + text + '](' + token.href + (title ? (' "' + title + '"') : '') + ')';
};
...
rules.image = function(tokens, idx/*, options, env */) {
  ...
  if (link || (token.src && token.src.indexOf('badge')) !== -1) {
    if (link && link.href) {
      return utils.mdu.badge(alt, src, link.href);
    }
    ...
    return utils.mdu.badge(alt, src, url);
  }
  return utils.mdu.image(alt, src, title);
};
...
rules.text = function(tokens, idx, options, env) {
  ...
  while (match = re.exec(token.content)) {
    var reflink = match[1];
    var text = match[2] || '';
    var anchor = match[3] || '';
    var inner = match[4] || '';
    ...
    if (ctx.reflinks && ctx.reflinks.hasOwnProperty(text)) {
      var resolved = reflink + '(' + ctx.reflinks[text] + inner + ')';
      var reflinkRe = new RegExp(utils.escapeRe(reflink) + '(?:\\[\\]|)' + utils.escapeRe(anchor));
      token.content = token.content.split(reflinkRe).join(resolved);
    }
  }
