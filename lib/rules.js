...
rules.code = function(tokens, idx /*, options, env */) {
  ...
  if (token.block) {
    if (!/^\n/.test(str)) str = '\n' + str;
    return '```' + str + '```' + getBreak(tokens, idx);
  }
  return '`' + str + '`';
};
...
rules.link_open = function(tokens, idx /*, options, env */) {
  ...
  if (close.type === 'link_close' && after && after.type === 'text') {
    var start = after.content.indexOf('{#');
    if (after.content && start === 0) {
      var end = after.content.indexOf('}');
      var href = after.content.slice(1, end);

      after.content = after.content.slice(end + 1).trim();
      if (after.content) {
